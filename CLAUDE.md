...
| DuckDB ≥ 0.10.0 | Couche analytique en mémoire — requêtes SQL directement sur les JSON natifs via `read_json_auto()` (stats sources, entités, sentiments, biais éditorial). Optionnel (`utils/db.py`) |
...
| Hourly 07:00–22:00 (`0 7-22 * * *`) | `watch_entity_articles.py` | Notify Discord of the latest freshly-detected article mentioning a watched entity — large image + AI-formatted summary. 1 article/run, **max 1 notification per entity per day**, notifications **only 07:00–22:00**, promotional/ad articles filtered out |
...
| 07:00 daily | `trend_detector.py` | Trend detection — generates `data/alertes.json`, notifies webhooks |
| 12:00 daily | `trend_detector.py` | Midday trend pass — same command; the cooldown suppresses already-notified entities so only new/escalated ones are pushed |
...
| `enrich_summary_format.py` | Round-robin enrichment — adds `Résumé_md` (Markdown formatté : chapitres `###`, gras/italique parcimonieux) à partir du `Résumé` brut, pour l'affichage enrichi. Reformatage local (Ollama) via `utils/summary_formatter.py` | `--flux`, `--keyword`, `--all`, `--since`, `--until`, `--dry-run`, `--force`, `--delay`, `--max-articles`, `--status` |
...
| `utils/api_client.py` | Triple-provider AI client (EurIA, Claude, Ollama) with retry/backoff logic — `generate_summary()`, `generate_entities()` (NER), `generate_sentiment()`, `generate_report()`. Provider selected via `AI_PROVIDER` env var. `get_ner_client()` factory routes NER/sentiment to `OllamaClient` when `AI_PROVIDER_NER=ollama`; `get_summary_client()` factory routes article summaries to `OllamaClient` when `AI_PROVIDER_SUMMARY=ollama`. Both fall back to cloud automatically. |
...
| `alert_rules.json` | Alert rule configuration — thresholds per entity type (modéré/élevé/critique), `global.laplace_k` (ratio smoothing, default 1.0), `global.cooldown_days` (webhook anti-repeat, default 2), webhook targets |
...
├── notified_alerts_state.json # Webhook cooldown state (TYPE:value → {date, niveau}) for trend_detector.py
├── watched_article_state.json # watch_entity_articles.py state: notified-article URLs + per-entity last-notified date (entity_daily) for the 1-notif/entity/day cap
├── watched_entities.json      # User watch list (entities tracked for alerts & hourly article watch)
